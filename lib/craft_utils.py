...
def getPoly_core(boxes, labels, mapper, linkmap):
    ...
    for k, box in enumerate(boxes):
        ...
        # make final polygon
        poly = []
        poly.append(warp_coordinates(Minv, (spp[0], spp[1])))
        for p in new_pp:
            poly.append(warp_coordinates(Minv, (p[0], p[1])))
        poly.append(warp_coordinates(Minv, (epp[0], epp[1])))
        poly.append(warp_coordinates(Minv, (epp[2], epp[3])))
        for p in reversed(new_pp):
            poly.append(warp_coordinates(Minv, (p[2], p[3])))
        poly.append(warp_coordinates(Minv, (spp[2], spp[3])))
...
def getDetBoxes(textmap, linkmap, text_threshold, link_threshold, low_text, poly=False, estimate_num_chars=False):
    if poly and estimate_num_chars:
        raise Exception(
            "Estimating not currently supported with poly.")
...
def coordinates_adjuster(polys, ratio_w, ratio_h, ratio_net=2):
    if len(polys) > 0:
